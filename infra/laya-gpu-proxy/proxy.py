...
# A stopped VM drops packets rather than refusing them, so an unreachable
# upstream costs the full deadline. Keep the predict deadline below the callers'
# own timeout, or they abandon the request before this 502 arrives.
...
async def predict(request: Request) -> Response:
    ...
    async with httpx.AsyncClient(timeout=PREDICT_TIMEOUT_SECONDS) as client:
        try:
